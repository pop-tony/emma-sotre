...
export default function Cart() {
  ...
  const payWithPaystack = () => {
    ...
    const handler = window.PaystackPop.setup({
      ...
      currency: 'GHS',
      ref: `EMMA_${checkoutItem.cartItemId}_${Date.now()}`,
      metadata: {
        custom_fields: [
          { display_name: "Customer Name", variable_name: "customer_name", value: customerInfo.name },
          { display_name: "Product", variable_name: "product", value: checkoutItem.name },
          { display_name: "Size", variable_name: "size", value: checkoutItem.size || 'N/A' },
          { display_name: "Color", variable_name: "color", value: checkoutItem.color || 'N/A' },
          { display_name: "Quantity", variable_name: "quantity", value: checkoutItem.quantity.toString() }
        ]
      },
                    ...
                    {cartItems.map((item) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="mt-3 flex w-full items-center justify-center gap-2 rounded-full bg-black py-2.5 text-sm font-semibold text-white transition hover:bg-neutral-800 active:scale-95 dark:bg-white dark:text-black dark:hover:bg-neutral-200"
                        >
          ...
          {checkoutStep === 'checkout' && checkoutItem && (
            <>
                ...
                <form className="space-y-4">
                  <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 dark:border-rose-400/20 dark:bg-rose-500/10">
                    ...
                    <div className="mt-1 flex gap-3 text-xs text-rose-800 dark:text-rose-300">
                      {checkoutItem.size && <span>Size: {checkoutItem.size}</span>}
                      <span>Qty: {checkoutItem.quantity}</span>
                    </div>
              ...
              <div className="border-t border-zinc-200 p-6 dark:border-zinc-800">
                ...
                >
                  Pay ₵{(checkoutItem.price * checkoutItem.quantity).toFixed(2)} with Paystack
                </button>
              </div>
            </>
          )}
